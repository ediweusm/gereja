...
**Validasi Balance (Debit = Kredit) — Dua Lapis**:
- **Lapis 1 – `rules()` pada Repeater**: Validasi standar Laravel yang menampilkan pesan kesalahan langsung di bawah komponen repeater.
- **Lapis 2 – `afterStateValidated()` pada Form**: Melempar `ValidationException` sebagai pengaman terakhir sebelum data disimpan ke database.
