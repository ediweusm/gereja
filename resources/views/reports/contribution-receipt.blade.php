@php
    ...
    $totalNominal = (float) $contribution->amount;
    $terbilangStr = ucwords(trim(terbilang($totalNominal))) . " Rupiah";
    $partyName = $contribution->member ? $contribution->member->fullName : '-';
    $transactionDate = $contribution->journal?->transaction_date 
        ? $contribution->journal->transaction_date->format('d-m-Y') 
        : now()->format('d-m-Y');
@endphp
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Tanda Terima Persembahan - {{ $contribution->journal?->transaction_number ?? 'REC-'.now()->format('YmdHis') }}</title>
    <style>
        @page {
            size: A5 landscape;
            margin: 8mm;
...
<div class="receipt-border">
    ...
    <table class="header-table">
        <tr>
            ...
            <td class="receipt-meta">
                <div class="receipt-title">Tanda Terima Persembahan</div>
                <div class="receipt-no">No: {{ $contribution->journal?->transaction_number ?? '-' }}</div>
            </td>
        </tr>
    </table>
    ...
    <table class="content-table">
        <tr>
            <td class="content-label">Telah Diterima Dari</td>
        ...
        <tr>
            <td class="content-label">Untuk Pembayaran</td>
            <td class="content-separator">:</td>
            <td class="content-value">
                <span style="color: #047857; text-transform: uppercase;">{{ $contribution->contributionType?->label ?? '-' }}</span>
                @if($contribution->journal?->description)
                    <span style="font-weight: 500; color: #4b5563;">({{ $contribution->journal->description }})</span>
                @endif
            </td>
        </tr>
    </table>
    ...
    <table class="bottom-table">
        <tr>
            ...
            <td style="width: 150px; text-align: center; vertical-align: bottom;">
                <!-- Left blank space/padding -->
            </td>
            <td class="signature-cell">
                <div>Diterima di: {{ str_replace('Jemaat ', '', $profile->church_name) }}</div>
                <div>Tanggal: {{ $transactionDate }}</div>
                <div class="signature-title">Bendahara Jemaat,</div>
